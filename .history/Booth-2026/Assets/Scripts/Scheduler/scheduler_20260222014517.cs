...
public sealed class Scheduler : MonoBehaviour
{
    ...
    private void ProcessDueEvents(float beatFloat, double songTime)
    {
        ...
        while ((nextScheduleIndex < Schedule.Events.Length) && (Schedule.Events[nextScheduleIndex].time <= songTime + scheduleAheadTime))
        {
            Schedule.Event evt = Schedule.Events[nextScheduleIndex];
            // spawn event
            Instantiate(evt.item, evt.transform.position, evt.transform.rotation);
