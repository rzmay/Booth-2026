...
public sealed class Scheduler : MonoBehaviour
{
    ...
    private void ProcessDueEvents(float beatFloat, double songTime)
    {
        ...
        while ((nextScheduleIndex < Schedule.Events.Length) && (isEventDue(Schedule.Events[nextScheduleIndex], beatFloat, songTime)))
        {
            ...
            obj = SpawnEvent(evt);
            InitializeSpawnedObj(obj, evt, beatFloat, songTimeSeconds);
    ...
    private void InitializeSpawnedObj(gameObject obj, Schedule.Event evt, float beatFloat, double songTime)
    {
